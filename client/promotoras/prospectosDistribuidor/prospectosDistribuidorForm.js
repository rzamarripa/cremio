...
.controller("ProspectosDistribuidorFormCtrl", ProspectosDistribuidorFormCtrl);
 function ProspectosDistribuidorFormCtrl($scope, $meteor, $reactive, $state, toastr, $stateParams){
...
*/
  ...
  if($stateParams.objeto_id != undefined){
      ...
      rc.objeto_id = $stateParams.objeto_id
			...
			Meteor.call('getProspectoDitribuidor', rc.objeto_id, function(error, result){           
            if (result)
            {
	            	console.log(result);
               	rc.objeto = result;
...
*/
      ...
      objeto.profile.avales_ids = rc.avales;
			...
			//Validar que no este dado de alta 
			Meteor.call('validaProspectoDitribuidor', objeto.profile.nombreCompleto, function(e,r){
          if (!r)
          {
	      			//console.log(r);  
	      			ProspectosDistribuidor.insert(objeto);
							toastr.succeess('Guardado correctamente');
							$state.go("root.ProspectosDistribuidorLista");  
          }
          else
          {
	          	//console.log(r);
	          	toastr.warning('ya existe un prospecto guardado');
          }
      });
...
*/
    ...
    _.each(objeto.profile.referenciasPersonales_ids, function(rp){	    
     ...
     delete objeto._id;
		 ...
		 ProspectosDistribuidor.update({_id: tempId}, {$set: objeto});
     toastr.succeess("Actualizado correctamente")
     $state.go("root.ProspectosDistribuidorLista");
    /*
loading(true);
