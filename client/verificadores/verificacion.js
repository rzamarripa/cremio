...
function VerificacionCtrl($scope, $meteor, $reactive, $state, $stateParams, toastr) {
		...
		this.subscribe('prospectosDistribuidor', () => {
		...
		},
			{
				onReady: function () {
					...
					var obj = ProspectosDistribuidor.findOne({ _id: $stateParams.id });

					if ($stateParams.persona == 1) {

						rc.objeto.cliente = {};
						rc.objeto.cliente.nombreCompleto = obj.profile.nombreCompleto;
						rc.objeto.cliente.calle = obj.profile.calle;
						rc.objeto.cliente.numero = obj.profile.numero;
						rc.objeto.cliente.codigoPostal = obj.profile.codigoPostal;
						rc.objeto.cliente.senasParticulares = obj.profile.senasParticulares;
						this.subscribe('colonias', () => {
							return [{ _id: obj.profile.colonia_id }]
						},
							{
								onReady: function () {
									rc.objeto.cliente.colonia = Colonias.findOne({ _id: obj.profile.colonia_id }).nombre;
									$scope.$apply();
								}
							});

						this.subscribe('ciudades', () => {
							return [{ _id: obj.profile.ciudad_id }]
						},
							{
								onReady: function () {
									rc.objeto.cliente.ciudad = Ciudades.findOne({ _id: obj.profile.ciudad_id }).nombre;
									$scope.$apply();
								}
							});

						rc.objeto.cliente.particular = obj.profile.particular;
						rc.objeto.cliente.celular = obj.profile.celular;

						$scope.$apply();
						rc.tipoClienteDistribuidor = "Prospecto";
					}
					else if ($stateParams.persona == 2) {
						if (obj != undefined) {
							Meteor.call('getAvalVerificacion', obj.profile.avales_ids[0]._id, function (error, result) {
								if (result) {
									rc.objeto.cliente = result;
									$scope.$apply();
								}
	...
	else if ($stateParams.tipo == "PCP") {
		this.subscribe('prospectosCreditoPersonal', () => {
			return [{ _id: $stateParams.id }]
		},
			{
				onReady: function () {
					...
					var obj = ProspectosCreditoPersonal.findOne({ _id: $stateParams.id });
					...
					if ($stateParams.persona == 1) {
						...
						this.subscribe('colonias', () => {
							...
							{
								onReady: function () {
									var col = Colonias.findOne({ _id: obj.profile.colonia_id });
									if (col != undefined) rc.objeto.cliente.colonia = col.nombre;
									$scope.$apply();
								}
							});
						...
						this.subscribe('ciudades', () => {
							...
							{
								onReady: function () {
									var ciu = Ciudades.findOne({ _id: obj.profile.ciudad_id });
									if (ciu != undefined) rc.objeto.cliente.ciudad = ciu.nombre;
									$scope.$apply();
								}
							});
						...
						rc.objeto.tipo = "Prospecto Crédito Personal";
						rc.objeto.capitalSolicitado = obj.profile.limiteCredito;
						$scope.$apply();
					}
					else if ($stateParams.persona == 2) {
						//var dis = Meteor.users.findOne({ _id: $stateParams.id });
	...
	this.guardar = function (obj, form) {
		...
		if ($stateParams.tipo == "PCP") {
			obj.cliente_id = $stateParams.id;
			obj.credito_id = "";
			obj.tipo = "Prospecto Crédito Personal";
			//delete obj.cliente;
			...
			obj.cliente_id = $stateParams.id;
			obj.tipo = "Prospecto Distribuidor";
			//delete obj.cliente;
		}
	...
	this.actualizar = function (obj, form) {
		...
		else if (obj.tipo == "Prospecto Crédito Personal") {
			delete obj.garantias;
			//delete obj.cliente;
			obj.garantias = angular.copy(this.garantias)
		}
		else if (obj.tipo == "Distribuidor") {
			//delete obj.cliente;
		}
	...
	this.editarGarantia = function (tipo, a) {
		...
		if (tipo == "mobiliaria") {
			...
			rc.garantia.num = a.num;
			rc.garantia.almacenaje = Number(a.almacenaje);
			rc.garantia.comercializacion = a.comercializacion;
			rc.garantia.desempenioExtemporaneo = a.desempenioExtemporaneo;
			rc.garantia.reposicionContrato = a.reposicionContrato;

			rc.garantia.descripcion = a.descripcion;
			rc.garantia.caracteristicas = a.caracteristicas;
			rc.garantia.avaluoMobiliaria = a.avaluoMobiliaria;
			rc.garantia.porcentajePrestamoMobiliria = a.porcentajePrestamoMobiliria;
	...
	this.calcularPorcentajeGeneral = function () {
		...
		if (rc.objeto.tipo == "Prospecto Crédito Personal") {
			if (rc.garantia.avaluoGeneral != undefined)
				rc.garantia.porcentajePrestamoGeneral = Math.round(rc.garantia.avaluoGeneral / rc.objeto.capitalSolicitado * 100);
			else
	...
	this.calcularPorcentajeMobiliaria = function () {
		if (rc.objeto.tipo == "Prospecto Crédito Personal") {
			if (rc.garantia.avaluoMobiliaria != undefined)
				rc.garantia.porcentajePrestamoMobiliria = Math.round(rc.garantia.avaluoMobiliaria / rc.objeto.capitalSolicitado * 100);
			else
